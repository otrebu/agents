...
function parseArgs(args: string[]): string {
  if (args.length === 0) {
    console.error('Usage: tsx create-plugin.ts <plugin-description>');
    console.error('Example: tsx create-plugin.ts "manages deployment workflows"');
    process.exit(1);
  }
...
function toKebabCase(text: string): string {
  const fillerWords = ['that', 'to', 'for', 'with', 'a', 'an', 'the', 'is', 'are', 'was', 'were'];
...
function checkPluginConflicts(pluginName: string, marketplacePath: string): ConflictCheck {
  if (!existsSync(marketplacePath)) {
    return { exists: false, message: 'Marketplace file not found, will create new entry' };
  }

  try {
    const marketplaceContent = readFileSync(marketplacePath, 'utf8');
    const marketplace = JSON.parse(marketplaceContent) as Marketplace;
    const existingPlugin = marketplace.plugins?.find(p => p.name === pluginName);

    if (existingPlugin) {
      return { exists: true, message: `Plugin "${pluginName}" already exists in marketplace` };
    }

    return { exists: false };
  } catch (err) {
    const errorMessage = err instanceof Error ? err.message : 'Unknown error';
    return { exists: false, message: `Error reading marketplace: ${errorMessage}` };
  }
}
...
\`\`\`bash
# Add a command
touch plugins/${pluginName}/commands/my-command.md

# Add an agent
mkdir -p plugins/${pluginName}/agents
touch plugins/${pluginName}/agents/my-agent.md
\`\`\`
...
function printSuccessMessage(pluginName: string): void {
  ...
  console.log(`  ✓ plugins/${pluginName}/.claude-plugin/plugin.json`);
  console.log(`  ✓ plugins/${pluginName}/commands/ (empty)`);
  console.log(`  ✓ plugins/${pluginName}/README.md`);
  console.log('  ✓ Updated .claude-plugin/marketplace.json');
  console.log();
  console.log('Next steps:');
  console.log(`  1. Add commands: Create .md files in plugins/${pluginName}/commands/`);
  ...
  console.log();
  console.log('The plugin is now registered in the marketplace and ready for development!');
}

// Main execution
function createPlugin(description: string, workingDirectory: string): void {
  const pluginName = toKebabCase(description);
  ...
  // Check for conflicts
  const marketplacePath = join(workingDirectory, '.claude-plugin', 'marketplace.json');
  ...
  const pluginPath = join(workingDirectory, 'plugins', pluginName);
...
try {
  ...
  createPlugin(description, workingDirectory);
} catch (err) {
  const errorMessage = err instanceof Error ? err.message : 'Unknown error';
